...
class ControllerCliente
{
    ...
    {
        try {
            $sql = "SELECT * FROM cliente ORDER BY nome"; // ORDENADO POR NOME
    ...
    private static function alterar(Cliente $cliente){
        try{
            $sql = "UPDATE cliente SET nome = :nome, email = :email, telefone = :telefone, endereco = :endereco, sexo = :sexo, profissao = :profissao WHERE id = :id";
            $p_sql = Conexao::getInstance()->prepare($sql);
            $p_sql->bindValue(":nome", $cliente->getNome());
            $p_sql->bindValue(":email", $cliente->getEmail());
            $p_sql->bindValue(":telefone", $cliente->getTelefone());
    ...
    {
        try {
            $sql = "SELECT * FROM cliente WHERE id = :id"; // FAÇO a consulta sql
            $p_sql = Conexao::getInstance()->prepare($sql);
            $p_sql->bindValue(":id", $id);
